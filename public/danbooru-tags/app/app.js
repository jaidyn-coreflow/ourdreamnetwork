﻿document.addEventListener('DOMContentLoaded', () => {
    ...
    const imageBasePath = isOnline 
        ? 'https://cdn.jsdelivr.net/gh/ThetaCursed/Illustrious-NoobAI-Style-Explorer@latest/'
        : '';
    ...
    function createCard(item) {
        ...
        card.addEventListener('click', (e) => {
            ...
            if (currentView === 'favorites' && e.ctrlKey) {
                ...
                navigator.clipboard.writeText(item.artist.replace(/\\/g, '')).then(() => {
                    showToast('Artist name copied — paste it into your prompt!');
                });
        ...
        try {
            ...
            if (typeof galleryData !== 'undefined' && allItems.length === 0) {
                ...
                allItems = galleryData.map(item => ({
                    artist: item.name.replace(/\\/g, ''), // strip SD-style backslash escapes — cleaner for Midjourney
