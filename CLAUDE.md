...
| `/rpg` | `rpg.html` | Role-playing game picker — tap one of 4 games → **email gate** → deep-link straight into an existing `ourdream.ai/chat/<slug>` group chat with `?ref=metarpg`. **Does not touch the create flow** (no Q-codes, no character build). Gate mirrors `fantasy.html`: fires `generate_lead` on valid email, `saveEmail` is fire-and-forget. Paid traffic appends `clickid`/`tracker=rt` (no `clk` hop). Games + their chat slugs are the `GAMES` array in the page. Card art is served from the Cloudflare R2 **Static** bucket (`static.ourdream.ai`, keys are **case-sensitive** — a casing typo is a silent 404): per game, `art` is the R2 still, `video` an R2 clip (`''` = still only), and `poster` a local JPEG in `public/assets/rpg/` used as the AVIF fallback and the video's first paint | Yes |
