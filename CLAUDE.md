...
| `/ai-character-maker` | `ai-character-maker.html` | Keyword LP; hero embeds the `/` quiz's step-1 style picker — picking Realistic/Anime hands off to `/?style=…`, which fires `quiz_start` and starts `/` at step 2 | Yes |
...
**`/ai-character-maker` follows the same no-email/no-RedTrack pattern**, built to the "Free AI Character Maker" copy doc (hero + 2 value props + 4-step build flow + privacy strip + FAQ, Google Ads-safe wording throughout — keep it that way). Its Step-1 section embeds the `/` quiz's step-1 style picker (same poster/video assets, videos via `constants.js` `cdnCreateVid`, deferred to `window.load`); each card links to `/?style=realistic|anime` + forwarded inbound params and fires `quiz_cta_clicked`. The hero/final/sticky-mobile CTAs are plain `data-quiz-cta` links to `/`. `index.html` consumes `?style=` on boot: fires `quiz_start`, deletes the param from `inboundParams`, and starts at step 2. Product screenshots live in `public/assets/ai-character-maker/` (staged SFW captures of ourdream.ai/create — see `shots/shot-manifest.md` while it exists).
